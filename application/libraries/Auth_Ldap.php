...
class Auth_Ldap {
    ...
    private function _init() {
		...
		$this->whitelist=$this->ci->config->item('whitelist');
		$this->user_whitelist=$this->ci->config->item('user_whitelist');
		$this->admin_groups=$this->ci->config->item('admin_groups');
    }
    ...
    function login($username, $password) {
        ...
        if (!$user_info)
			...
			foreach ($this->whitelist as $group){
				if ($this->ci->utilities->substr_in_array($user_info['groups'], $group)){
					$group_found=true;
					break;
				}
			}
			
			foreach ($this->user_whitelist as $user){
				if ($user_info['id'] === $user){
					$group_found=true;
					break;
				}
			}
			foreach ($this->admin_groups as $group){
				if ($this->ci->utilities->substr_in_array($user_info['groups'], $group)){
					$admin=True;
				}
			}
		...
		if (in_array($username, $this->ci->config->item('admins')))
    ...
    private function _authenticate($username, $password) {
        $needed_attrs = array('dn', 'cn', $this->login_attribute, 'memberOf', 'mail');
        ...
        if($this->use_ad) {
         ...
         return array('cn' => $cn, 'dn' => $dn, 'id' => $id, 'groups'=>$groups, 'mail'=>$mail,
